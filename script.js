    ...
    function updateMilestoneLabel() {
      const age = getAge(birthDate);
      const monthsSinceBirth = (new Date().getFullYear() - birthDate.getFullYear()) * 12 + (new Date().getMonth() - birthDate.getMonth());
      let label;
    ...
    function updateAge() {
      const age = getAge(birthDate);
      document.getElementById("baby-age").textContent = `${age.years} year${age.years !== 1 ? 's' : ''} and ${age.days} day${age.days !== 1 ? 's' : ''}`;
    }
...
// -------------------------------------
document.querySelectorAll(".overlayadd .overlay-header img, .overlayedit .overlay-header img, .overlayupload .overlay-header img")
  .forEach(closeBtn => {
...
// -------------------------------------
document.querySelectorAll(".overlayadd .save-button, .overlayedit .save-button, .overlayupload .upload-button")
  .forEach(saveBtn => {
...
// -------------------------------------
document.querySelectorAll(".overlayadd, .overlayedit, .overlayupload")
  .forEach(overlay => {
